...
class VariationProduct extends Model
{
    ...
    public static function getDuplicateRows()
    {
        return self::select('code', DB::raw('COUNT(*) as count'))
            ->where("clientCode", "605325")
            ->groupBy('code')
            ->having('count', '>', 1)
            ->get();
    }
    ...
    static public function deleteProduct($clientCode, $productID){
        ...
        if($old){
            ...
            UserLogger::setChronLogNew($old ? json_encode($old, true) : '', json_encode($change, true), "Variation Product Deleted");    
        }
    }
