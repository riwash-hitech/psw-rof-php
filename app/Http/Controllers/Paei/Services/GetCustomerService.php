...
class GetCustomerService implements UserOperationInterface
{
    ...
    public function saveUpdate($customers, $isAdded = 0)
    {

        $clientCode = $this->api->client->clientCode;
        $customerIDs = collect($customers)->pluck('customerID')->all();
        $existing = $this->customer->where('clientCode', $clientCode)
        ...
        foreach ($customers as $c) {
            try {
                $this->saveUpdateCustomerOldApi($c, false, 0, $existing->get($c['customerID']));
            } catch (\Throwable $e) {
                info("Customer Sync Failed for customerID " . @$c['customerID'] . ": " . $e->getMessage());
            }
        }
        ...
        if ($isAdded == 1) {
            ...
            ErplySync::where("id", 1)->update(["psw_customer_modified" => date('Y-m-d H:i:s', $forUpdate['lastModified'])]);
        }
    ...
    public function saveUpdateBulk($customers, $isAdded = 0)
    {
        ...
        foreach ($customers as $c) {
            try {
                $this->saveUpdateCustomer($c, false, 0, $existing->get($c['id']));
            } catch (\Throwable $e) {
                info("Customer Bulk Sync Failed for customerID " . @$c['id'] . ": " . $e->getMessage());
            }
        }
        ...
        if ($isAdded == 1) {
            ...
            ErplySync::where("id", 1)->update(["psw_customer_modified" => date('Y-m-d H:i:s', $forUpdate['changed'])]);
        }

        return response()->json(['status' => 200, 'message' => "Customer fetched Successfully."]);
    }

    public function saveUpdateCustomer($product, $isIndividual = false, $clientCode = 0, $old = null)
    {
        ...
        $oldSnapshot = $old ? clone $old : null;
        if ($old) {
            $old->fill($values)->save();
            $change = $old;
        } else {
            $change = $this->customer->create($values);
        }
        $this->letsLog->setChronLog($oldSnapshot ? json_encode($oldSnapshot, true) : '', json_encode($change, true), $oldSnapshot  ? "Customer Updated" : "Customer Created");
    }

    public function saveUpdateCustomerOldApi($product, $isIndividual = false, $clientCode = 0, $old = null)
    {
        if ($old === null) {
    ...
    public function getLastUpdateDate($isAdded)
    {
        ...
        $erplySync = ErplySync::where("id", 1)->first();
        $latest = $isAdded == 1 ? $erplySync->psw_customer_added : $erplySync->psw_customer_modified;
        if (empty($latest) || $latest == "0000-00-00 00:00:00.000") {
            return 0;
        }
